(function() {
  var Project, Promise, Reporter, TITLE_SEPARATOR, Windows, _, browsers, chalk, collectTestResults, errors, fs, haveProjectIdAndKeyButNoRecordOption, human, humanTime, log, openProject, path, pkg, progress, random, ss, stats, terminal, trash, user, uuid, video;
  ...
  module.exports = {
    ...
    postProcessRecording: function(end, name, cname, videoCompression) {
      return end().then(function() {
        var onProgress, started, tenSecs;
        if (videoCompression === false) {
          return;
        }
        console.log("");
        console.log("");
    ...
    launchBrowser: function(options) {
      var browser, browserOpts, gui, project, screenshots, spec, write;
      if (options == null) {
        options = {};
      }
      browser = options.browser, spec = options.spec, write = options.write, gui = options.gui, project = options.project, screenshots = options.screenshots;
      gui = !!gui;
      if (browser == null) {
        browser = "electron";
      }
      browserOpts = (function() {
        switch (browser) {
          case "electron":
            return this.getElectronProps(gui, project, write);
          default:
            return {};
        }
      }).call(this);
    ...
    waitForTestsToFinishRunning: function(options) {
      var cname, end, gui, name, outputPath, project, screenshots, started, videoCompression;
      if (options == null) {
        options = {};
      }
      project = options.project, gui = options.gui, screenshots = options.screenshots, started = options.started, end = options.end, name = options.name, cname = options.cname, videoCompression = options.videoCompression, outputPath = options.outputPath;
      return this.listenForProjectEnd(project, gui).then((function(_this) {
        return function(obj) {
          var finish, ft, testResults, writeOutput;
          if (end) {
            obj.video = name;
          }
          if (screenshots) {
          ...
          if (end) {
            return _this.postProcessRecording(end, name, cname, videoCompression).then(finish);
          } else {
            return finish();
          }
        };
    ...
    copy: function(videosFolder, screenshotsFolder) {
      return Promise["try"](function() {
        var ca;
        if ((ca = process.env.CIRCLE_ARTIFACTS) && process.env["COPY_CIRCLE_ARTIFACTS"] !== "false") {
          return Promise.join(ss.copy(screenshotsFolder, ca), video.copy(videosFolder, ca));
        }
      });
    },
    allDone: function() {
