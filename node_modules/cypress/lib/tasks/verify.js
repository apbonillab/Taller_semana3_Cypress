...
var maybeVerify = function maybeVerify() {
  ...
  return info.getVerifiedVersion().then(function (verifiedVersion) {
    ...
    var shouldVerify = options.force || differentFrom(packageVersion, verifiedVersion);

    debug('run verification check?', shouldVerify);

    if (shouldVerify) {
      return testBinary(packageVersion).then(function () {
